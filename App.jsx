...
const assessmentResults = [
  ...
  { lo: 12, hi: 14, title: "The Clearing", icon: "\uD83C\uDF3F", desc: "You're finding your footing. Some days are still brutal, but you're starting to understand that your healing matters too. You're ready for deeper work, building boundaries that are sustainable and a life that holds space for love AND self-preservation.", color: C.sage, bg: C.sageBg, next: ["Advanced Boundaries Toolkit", "Do What You Can Live With (The Book)", "The Crossroads Model Deep Dive"] },
  { lo: 15, hi: 16, title: "The Path", icon: "\uD83E\uDDED", desc: "You've done real work on yourself and how you show up. Boundaries feel like acts of love now, not punishment. You're navigating, not just surviving. Your experience could be a lifeline for someone who's earlier in this journey.", color: C.taupe, bg: C.taupeBg, next: ["Mentor Training Program", "Share Your Story", "Professional Advocacy Resources"] },
];
...
function Hero({ onAssess, onBound }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "120px 24px 80px", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse at 20% 30%, ${C.sageBg} 0%, transparent 50%), radial-gradient(ellipse at 80% 70%, ${C.warmBg} 0%, transparent 50%), ${C.bg}` }} />
      <div style={{ position: "relative", zIndex: 1, maxWidth: 780, margin: "0 auto", opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(24px)", transition: "all 1s ease-out" }}>
        <p style={{ fontFamily: SN, fontSize: 13, letterSpacing: 3, color: C.sage, textTransform: "uppercase", fontWeight: 600, marginBottom: 20 }}>For families who refuse to give up</p>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          <button onClick={onAssess} style={{ fontFamily: SN, padding: "16px 32px", background: C.sage, color: "#fff", border: "none", borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "all 0.25s", boxShadow: `0 2px 12px ${C.sage}44` }} onMouseOver={e => e.target.style.background = C.dark} onMouseOut={e => e.target.style.background = C.sage}>
            Find Where You Are
          </button>
          <button onClick={onBound} style={{ fontFamily: SN, padding: "16px 32px", background: "transparent", color: C.warm, border: `2px solid ${C.warm}`, borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "all 0.25s" }} onMouseOver={e => e.target.style.background = C.warmBg} onMouseOut={e => e.target.style.background = "transparent"}>
            Build a Boundary
          </button>
        </div>
      </div>
...
function CredBar() {
  return (
    <div style={{ padding: "36px 24px", background: C.bgAlt, borderTop: `1px solid ${C.bdr}`, borderBottom: `1px solid ${C.bdr}` }}>
      <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 20, textAlign: "center" }}>
        {credBadges.map((b, i) => (
          <div key={i} style={{ padding: "8px 0" }}>
            <div style={{ fontFamily: SN, fontSize: 15, color: C.txt, fontWeight: 700 }}>{b.label}</div>
            <div style={{ fontFamily: SN, fontSize: 13, color: C.txtLt, marginTop: 3 }}>{b.detail}</div>
          </div>
        ))}
      </div>
    </div>
...
function Sec({ children, dark, alt, id }) {
  return (
    <div id={id} style={{ padding: "80px 24px", background: dark ? C.dark : alt ? C.bgAlt : C.bg, borderBottom: dark ? "none" : `1px solid ${C.bdrLt}` }}>
      <div style={{ maxWidth: 800, margin: "0 auto" }}>{children}</div>
    </div>
  );
}

function SH({ label, title, sub, dark }) {
  return (
    <div style={{ textAlign: "center", marginBottom: 48 }}>
      {label && <p style={{ fontFamily: SN, fontSize: 12, letterSpacing: 3, color: dark ? C.sageMd : C.sage, textTransform: "uppercase", fontWeight: 600, marginBottom: 12 }}>{label}</p>}
      <h2 style={{ fontFamily: SF, fontSize: "clamp(28px, 5vw, 42px)", color: dark ? C.txtW : C.txt, fontWeight: 600, margin: "0 0 12px", lineHeight: 1.2 }}>{title}</h2>
      {sub && <p style={{ fontFamily: SN, fontSize: 16, color: dark ? C.txtWlt : C.txtMd, maxWidth: 560, margin: "0 auto", lineHeight: 1.65 }}>{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div onClick={() => setFlipped(!flipped)} style={{ cursor: "pointer", perspective: 1000, minHeight: 300, marginBottom: 16 }}>
      <div style={{ position: "relative", width: "100%", minHeight: 300, transition: "transform 0.6s", transformStyle: "preserve-3d", transform: flipped ? "rotateY(180deg)" : "rotateY(0)" }}>
        ...
        <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", transform: "rotateY(180deg)", background: C.sageBg, border: `1px solid ${C.sageMd}`, borderRadius: 12, padding: 28, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <p style={{ fontFamily: SN, fontSize: 11, letterSpacing: 3, color: C.sage, textTransform: "uppercase", marginBottom: 12, fontWeight: 700 }}>What we actually know</p>
          <p style={{ fontFamily: SN, fontSize: 15, color: C.txt, lineHeight: 1.6, margin: "0 0 16px" }}>{truth}</p>
          <div style={{ background: `${C.sage}18`, borderRadius: 8, padding: 12 }}>
            <p style={{ fontFamily: SN, fontSize: 13, color: C.dark, margin: 0, lineHeight: 1.5, fontWeight: 500 }}>{stat}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      <div style={{ height: 3, background: C.bdr, borderRadius: 2, marginBottom: 32 }}>
        <div style={{ height: "100%", background: `linear-gradient(90deg, ${C.sage}, ${C.taupe})`, borderRadius: 2, width: `${(step / assessmentQs.length) * 100}%`, transition: "width 0.5s" }} />
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {q.opts.map((o, i) => (
          <button key={i} onClick={() => setSel(o.score)} style={{
            fontFamily: SN, padding: "16px 20px", background: sel === o.score ? C.sageBg : C.card,
            border: `1.5px solid ${sel === o.score ? C.sage : C.bdr}`, borderRadius: 10,
            color: sel === o.score ? C.dark : C.txtMd, fontSize: 15, textAlign: "left",
...
function AssessmentResult({ score, onReset }) {
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
      ...
      <div style={{ textAlign: "left", background: C.card, borderRadius: 12, padding: 24, marginBottom: 24, border: `1px solid ${C.bdr}` }}>
        <p style={{ fontFamily: SN, fontSize: 12, color: C.sage, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>Recommended Next Steps</p>
        {r.next.map((n, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: i < r.next.length - 1 ? `1px solid ${C.bdrLt}` : "none" }}>
            <div style={{ width: 26, height: 26, borderRadius: "50%", background: C.sageBg, color: C.sage, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{i + 1}</div>
...
function BoundaryBuilder() {
  ...
  const btn = (text, onClick) => (
    <button onClick={onClick} style={{
      fontFamily: SN, padding: "16px 20px", background: C.card,
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <p style={{ fontFamily: SN, fontSize: 13, color: C.taupe, marginBottom: 20, fontWeight: 500 }}>Fear: {fear}</p>
      <h3 style={{ fontFamily: SF, fontSize: 22, color: C.txt, marginBottom: 20 }}>What Boundaried Love Sounds Like</h3>
      <div style={{ background: C.warmBg, border: `1px solid ${C.warmMd}`, borderRadius: 12, padding: 28 }}>
        <p style={{ fontFamily: SF, fontSize: 18, color: C.txt, lineHeight: 1.7, margin: 0, fontStyle: "italic" }}>{getResp()}</p>
      </div>
      <div style={{ marginTop: 20, padding: 20, background: C.sageBg, borderRadius: 10 }}>
        <p style={{ fontFamily: SN, fontSize: 14, color: C.dark, fontWeight: 600, marginBottom: 6 }}>Remember:</p>
    ...
    <Sec alt>
      <div style={{ display: "flex", gap: 48, alignItems: "center", flexWrap: "wrap", justifyContent: "center" }}>
        <div style={{ flexShrink: 0 }}>
          <img
            src={bookCover}
            alt="Do What You Can Live With by Brandi Mac"
            style={{ width: 260, borderRadius: 8, boxShadow: "0 8px 32px rgba(58,52,41,0.15)" }}
          />
        </div>
        <div style={{ flex: 1, minWidth: 280 }}>
          <p style={{ fontFamily: SN, fontSize: 12, letterSpacing: 3, color: C.sage, textTransform: "uppercase", fontWeight: 600, marginBottom: 12 }}>The Book</p>
          <h2 style={{ fontFamily: SF, fontSize: "clamp(26px, 4vw, 36px)", color: C.txt, fontWeight: 600, margin: "0 0 16px", lineHeight: 1.2 }}>Do What You Can Live With</h2>
          <p style={{ fontFamily: SN, fontSize: 15, color: C.txtMd, lineHeight: 1.7, marginBottom: 12 }}>
            A Survival Guide for Families Navigating Addiction, Grief, and Impossible Choices.
          </p>
          <p style={{ fontFamily: SN, fontSize: 15, color: C.txtMd, lineHeight: 1.7, marginBottom: 12 }}>
            This is not a workbook about control. It's a workbook about peace. About making decisions you can live with. About doing the best you can with a broken heart and not losing yourself in the process.
          </p>
          <p style={{ fontFamily: SN, fontSize: 14, color: C.sage, fontWeight: 600, marginBottom: 20 }}>
            On Caron Treatment Centers' Recommended Reading List
          </p>
          <button style={{ fontFamily: SN, padding: "14px 28px", background: C.sage, color: "#fff", border: "none", borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "all 0.25s" }}
            onMouseOver={e => e.target.style.background = C.dark}
            onMouseOut={e => e.target.style.background = C.sage}>
            Get the Book
          </button>
        </div>
      </div>
...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, color: C.txt, fontFamily: SN, minHeight: "100vh" }}>
      {/* Nav */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: `${C.bg}F0`, backdropFilter: "blur(16px)", borderBottom: `1px solid ${C.bdr}`, padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontFamily: SF, fontSize: 16, fontWeight: 700, color: C.txt }}>Do What You Can Live With</div>
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "flex-end" }}>
          {["The Book", "The Program", "Resources", "For Professionals"].map(t => (
            <button key={t} style={{ fontFamily: SN, padding: "6px 12px", background: "transparent", color: C.txtMd, border: "none", fontSize: 13, cursor: "pointer", borderRadius: 6, fontWeight: 500 }}>{t}</button>
          ))}
        </div>
      </nav>

      <Hero
        onAssess={() => assessRef.current?.scrollIntoView({ behavior: "smooth" })}
        onBound={() => boundRef.current?.scrollIntoView({ behavior: "smooth" })}
      />
      <CredBar />
      ...
      <Sec>
        <div style={{ maxWidth: 640, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontFamily: SN, fontSize: 16, color: C.txtMd, lineHeight: 1.7 }}>
            That false choice is destroying families and costing lives. This space exists because no one gave you a way to do both. Until now.
          </p>
        </div>
      </Sec>
      ...
      <Sec id="assessment">
        <div ref={assessRef} />
        <SH label="Find Where You Are" title="This Isn't a Test. It's a Compass." sub="Answer honestly. There are no wrong answers and no grades. We just want to meet you exactly where you are and point you toward the tools that will actually help." />
        {assessScore === null ? <Assessment onComplete={setAssessScore} /> : <AssessmentResult score={assessScore} onReset={() => setAssessScore(null)} />}
      </Sec>

      {/* Boundary Builder */}
      <Sec alt id="boundaries">
        <div ref={boundRef} />
      ...
      {/* Myths */}
      <Sec>
        <SH label="Evidence Over Tradition" title="What You've Been Told vs. What's Actually True" sub="Tap each card to flip it. The advice you received was never evidence-based. Here's what the research actually says." />
      ...
      <Sec dark>
        <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
          <p style={{ fontFamily: SN, fontSize: 12, letterSpacing: 3, color: C.sageMd, textTransform: "uppercase", fontWeight: 600, marginBottom: 20 }}>Permission Granted</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {["You are allowed to still love them.", "You are allowed to be angry and heartbroken at the same time.", "You are allowed to set a boundary and cry about it later.", "You are allowed to hope and grieve simultaneously.", "You are allowed to not have this figured out.", "You are allowed to take care of yourself without guilt."].map((p, i) => (
              <p key={i} style={{ fontFamily: SF, fontSize: "clamp(18px, 3vw, 24px)", color: C.txtW, lineHeight: 1.4, margin: 0, fontWeight: 400, opacity: 0.92 }}>{p}</p>
            ))}
          </div>
        </div>
      </Sec>
      ...
      <Sec alt>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
          {resources.map((r, i) => (
            <div key={i} style={{ background: C.card, borderRadius: 12, padding: 24, border: `1px solid ${C.bdr}`, transition: "all 0.25s" }}>
              <h4 style={{ fontFamily: SF, fontSize: 18, color: C.txt, fontWeight: 600, marginBottom: 16 }}>{r.cat}</h4>
              {r.items.map((item, j) => (
      ...
      {/* Community */}
      <Sec>
        <SH label="You're Not Alone in This" title="400,000+ Families. One Truth." sub="You can love someone with addiction and survive it. Here's how to stay connected." />
        <div style={{ maxWidth: 700, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
          {[
            ...
            <div key={i} style={{ background: C.card, borderRadius: 12, padding: 24, border: `1px solid ${C.bdr}`, cursor: "pointer", transition: "all 0.25s" }}>
              <h4 style={{ fontFamily: SF, fontSize: 18, color: C.txt, fontWeight: 600, marginBottom: 8 }}>{c.title}</h4>
              <p style={{ fontFamily: SN, fontSize: 14, color: C.txtMd, lineHeight: 1.6, margin: 0 }}>{c.desc}</p>
            </div>
          ))}
        </div>
      </Sec>
      ...
      <Sec alt>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto" }}>
          <div style={{ background: C.card, border: `1px solid ${C.bdr}`, borderRadius: 16, padding: 36 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 24, marginBottom: 32 }}>
              {[
                ...
                <div key={i}>
                  <h4 style={{ fontFamily: SF, fontSize: 17, color: C.warm, fontWeight: 600, marginBottom: 8 }}>{s.title}</h4>
                  <p style={{ fontFamily: SN, fontSize: 14, color: C.txtMd, lineHeight: 1.55, margin: 0 }}>{s.desc}</p>
                </div>
              ))}
            </div>
            <div style={{ textAlign: "center" }}>
              <button style={{ fontFamily: SN, padding: "16px 36px", background: C.dark, color: "#fff", border: "none", borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: "pointer" }}>Partner With Us</button>
      ...
      <Sec dark>
        <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
          <p style={{ fontFamily: SF, fontSize: "clamp(24px, 5vw, 36px)", color: C.txtW, lineHeight: 1.4, margin: "0 0 16px", fontWeight: 400, fontStyle: "italic" }}>
            "If they died tomorrow, would I be at peace with how I loved them today?"
          </p>
          <p style={{ fontFamily: SN, fontSize: 15, color: C.txtWlt, lineHeight: 1.6 }}>
            That's the only question that matters. Everything we build here helps you answer it.
          </p>
        </div>
      </Sec>

      {/* Footer */}
      <footer style={{ padding: "48px 24px", textAlign: "center", background: C.bg, borderTop: `1px solid ${C.bdr}` }}>
        <p style={{ fontFamily: SF, fontSize: 22, color: C.txt, fontWeight: 600, marginBottom: 8 }}>Do What You Can Live With</p>
        <p style={{ fontFamily: SN, fontSize: 14, color: C.txtLt, maxWidth: 440, margin: "0 auto 24px", lineHeight: 1.6 }}>Because love and boundaries were never meant to be opposites.</p>
        <div style={{ display: "flex", justifyContent: "center", gap: 24, marginBottom: 24 }}>
          {["Facebook", "Instagram", "TikTok", "Podcast"].map(s => (
            <span key={s} style={{ fontFamily: SN, fontSize: 13, color: C.txtLt, cursor: "pointer" }}>{s}</span>
          ))}
        </div>
        <p style={{ fontFamily: SN, fontSize: 12, color: C.txtLt }}>© 2025 Brandi McCurdy. All rights reserved.</p>
      </footer>
    </div>
  );
}
