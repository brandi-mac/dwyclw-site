...
const assessmentResults = [
  ...
  { lo: 12, hi: 14, title: "The Clearing", icon: "\uD83C\uDF3F", desc: "You're finding your footing. Some days are still brutal, but you're starting to understand that your healing matters too. You're ready for deeper work, building boundaries that are sustainable and a life that holds space for love AND self-preservation.", color: C.green, bg: C.greenPale, next: ["Advanced Boundaries Toolkit", "Do What You Can Live With (The Book)", "The Crossroads Model Deep Dive"] },
  { lo: 15, hi: 16, title: "The Path", icon: "\uD83E\uDDED", desc: "You've done real work on yourself and how you show up. Boundaries feel like acts of love now, not punishment. You're navigating, not just surviving. Your experience could be a lifeline for someone who's earlier in this journey.", color: C.green, bg: C.greenLt, next: ["Mentor Training Program", "Share Your Story", "Professional Advocacy Resources"] },
];
...
function Hero({ onAssess, onBound }) {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", padding: "120px 24px 80px", position: "relative", overflow: "hidden", background: C.green }}>
      <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse at 20% 30%, ${C.greenMd}44 0%, transparent 50%), radial-gradient(ellipse at 80% 70%, ${C.greenDk}44 0%, transparent 50%), ${C.green}` }} />
      <div style={{ position: "relative", zIndex: 1, maxWidth: 780, margin: "0 auto", opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(24px)", transition: "all 1s ease-out" }}>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          <button onClick={onAssess} style={{ fontFamily: SN, padding: "16px 32px", background: C.white, color: C.green, border: "none", borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "all 0.25s" }} onMouseOver={e => e.target.style.background = C.cream} onMouseOut={e => e.target.style.background = C.white}>
            Find Where You Are
          </button>
          <button onClick={onBound} style={{ fontFamily: SN, padding: "16px 32px", background: "transparent", color: C.white, border: "2px solid rgba(255,255,255,0.5)", borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: "pointer", transition: "all 0.25s" }} onMouseOver={e => e.target.style.borderColor = "rgba(255,255,255,0.9)"} onMouseOut={e => e.target.style.borderColor = "rgba(255,255,255,0.5)"}>
            Build a Boundary
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      <div style={{ height: 3, background: C.greenLt, borderRadius: 2, marginBottom: 32 }}>
        <div style={{ height: "100%", background: C.green, borderRadius: 2, width: `${(step / assessmentQs.length) * 100}%`, transition: "width 0.5s" }} />
      </div>
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <div style={{ marginTop: 20, padding: 20, background: C.greenTint, borderRadius: 10, border: `1px solid ${C.greenLt}` }}>
        <p style={{ fontFamily: SN, fontSize: 14, color: C.greenDk, fontWeight: 600, marginBottom: 6 }}>Remember:</p>
...
export default function App() {
  ...
  return (
    <div style={{ background: C.cream, color: C.txt, fontFamily: SN, minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ display: "flex", gap: 48, alignItems: "center", flexWrap: "wrap", justifyContent: "center" }}>
            <div style={{ flexShrink: 0 }}>
              <img src={bookCover} alt="Do What You Can Live With by Brandi Mac" style={{ width: 260, borderRadius: 8, boxShadow: "0 8px 32px rgba(0,0,0,0.3)" }} />
            </div>
            <div style={{ flex: 1, minWidth: 280 }}>
              <p style={{ fontFamily: SN, fontSize: 12, letterSpacing: 3, color: C.greenLt, textTransform: "uppercase", fontWeight: 600, marginBottom: 12 }}>The Book</p>
              ...
              <p style={{ fontFamily: SN, fontSize: 15, color: C.greenLt, lineHeight: 1.7, marginBottom: 12 }}>A Survival Guide for Families Navigating Addiction, Grief, and Impossible Choices.</p>
              <p style={{ fontFamily: SN, fontSize: 15, color: C.greenLt, lineHeight: 1.7, marginBottom: 12 }}>This is not a workbook about control. It's a workbook about peace. About making decisions you can live with. About doing the best you can with a broken heart and not losing yourself in the process.</p>
              ...
              <button style={{ fontFamily: SN, padding: "14px 28px", background: C.white, color: C.green, border: "none", borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: "pointer" }}>Get the Book</button>
            </div>
          </div>
        </div>
      </div>

      {/* Assessment - cream (breather) */}
      <div style={{ padding: "80px 24px", background: C.cream }} id="assessment">
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <p style={{ fontFamily: SN, fontSize: 16, color: C.txtMd, maxWidth: 560, margin: "0 auto", lineHeight: 1.65 }}>Answer honestly. There are no wrong answers and no grades. We just want to meet you exactly where you are and point you toward the tools that will actually help.</p>
          </div>
          {assessScore === null ? <Assessment onComplete={setAssessScore} /> : <AssessmentResult score={assessScore} onReset={() => setAssessScore(null)} />}
        </div>
      </div>

      {/* Boundary Builder - green wash */}
      <div style={{ padding: "80px 24px", background: C.greenWash }} id="boundaries">
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <p style={{ fontFamily: SN, fontSize: 16, color: C.txtMd, maxWidth: 560, margin: "0 auto", lineHeight: 1.65 }}>Boundaries aren't walls. They're bridges with guardrails. Let's build one together that feels like love, not punishment.</p>
          </div>
          <BoundaryBuilder />
        </div>
      </div>
      ...
      <div style={{ padding: "80px 24px", background: C.green }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: SF, fontSize: "clamp(28px, 5vw, 42px)", color: C.white, fontWeight: 600, margin: "0 0 12px", lineHeight: 1.2 }}>What You've Been Told vs. What's Actually True</h2>
            <p style={{ fontFamily: SN, fontSize: 16, color: C.greenLt, maxWidth: 560, margin: "0 auto", lineHeight: 1.65 }}>Tap each card to flip it. The advice you received was never evidence-based. Here's what the research actually says.</p>
          </div>
        ...
        <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {["You are allowed to still love them.", "You are allowed to be angry and heartbroken at the same time.", "You are allowed to set a boundary and cry about it later.", "You are allowed to hope and grieve simultaneously.", "You are allowed to not have this figured out.", "You are allowed to take care of yourself without guilt."].map((p, i) => (
              <p key={i} style={{ fontFamily: SF, fontSize: "clamp(18px, 3vw, 24px)", color: C.greenTint, lineHeight: 1.4, margin: 0, fontWeight: 400, opacity: 0.92 }}>{p}</p>
            ))}
          </div>
        </div>
      </div>

      {/* Resources - green pale */}
      <div style={{ padding: "80px 24px", background: C.greenPale }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <p style={{ fontFamily: SN, fontSize: 16, color: C.txtMd, maxWidth: 560, margin: "0 auto", lineHeight: 1.65 }}>Whether it's 2am and you're in crisis, or it's Tuesday and you just need someone to tell you you're doing okay.</p>
          </div>
          <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
            {resources.map((r, i) => (
              <div key={i} style={{ background: C.white, borderRadius: 12, padding: 24, border: `1px solid ${C.greenLt}` }}>
      ...
      <div style={{ padding: "80px 24px", background: C.green }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <p style={{ fontFamily: SN, fontSize: 16, color: C.greenLt, maxWidth: 560, margin: "0 auto", lineHeight: 1.65 }}>You can love someone with addiction and survive it. Here's how to stay connected.</p>
          </div>
          <div style={{ maxWidth: 700, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
            {[
              ...
              <div key={i} style={{ background: `${C.white}18`, borderRadius: 12, padding: 24, border: `1px solid ${C.greenMd}`, cursor: "pointer", backdropFilter: "blur(4px)" }}>
      ...
      {/* For Professionals - cream (breather) */}
      <div style={{ padding: "80px 24px", background: C.cream }}>
        <div style={{ maxWidth: 800, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <p style={{ fontFamily: SN, fontSize: 16, color: C.txtMd, maxWidth: 560, margin: "0 auto", lineHeight: 1.65 }}>Evidence-based family curriculum, staff training, and speaking engagements that change how your organization approaches family involvement.</p>
          </div>
          <div style={{ maxWidth: 700, margin: "0 auto" }}>
      ...
      <div style={{ padding: "80px 24px", background: C.greenDk }}>
        <div style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
          ...
          <p style={{ fontFamily: SN, fontSize: 15, color: C.greenMist, lineHeight: 1.6 }}>That's the only question that matters. Everything we build here helps you answer it.</p>
        </div>
      </div>

      {/* Footer - green */}
      <footer style={{ padding: "48px 24px", textAlign: "center", background: C.green }}>
